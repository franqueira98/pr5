...
public class newVehicleEvent extends Event {
	...
	public newVehicleEvent(int time,List <Junction>itinerary,int maxSpeed,String id) {
		super(time);
		...
		public Event parse(IniSection ini){
			if(!ini.getTag().equals("new_vehicle")){
				...
				for(int i=0;i<aux.length();i++){
					//hace falta acceso a roadMap para pasar el Junction a partir del id.
					if(aux.charAt(i)!=',')
						idJunc+=aux.charAt(i);
					else{
						//it.add(c)//aqui estaría el id guardado y solo habria que llamar al metodo
